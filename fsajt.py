...
def check_and_install_packages(package_names):
    for package in package_names:
        try:
            subprocess.check_call([sys.executable, "-m", "pip", "install", "--use-deprecated=backtrack-on-build-failures", f"{package}"])
        except subprocess.CalledProcessError as e:
            print(f"Failed to install {package} due to {e}, trying to install older versions.")
            for version_offset in range(1, 6):
                try:
                    subprocess.check_call([sys.executable, "-m", "pip", "install", f"{package}~={version_offset}"])
                    break
                except subprocess.CalledProcessError:
                    if version_offset == 5:
                        print(f"Failed to install an older version of {package}. Please check the package compatibility.")
                        sys.exit(1)
